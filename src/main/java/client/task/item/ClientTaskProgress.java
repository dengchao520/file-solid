...
public class ClientTaskProgress implements ClientTaskManageItem {
    ...
    public void exec(String taskName) {
        if (!ClientTaskCache.getTaskMap().containsKey(taskName)) {
            ...
            System.out.println(taskName + "--已经传输---" + ClientTaskCache.getTaskMap().get(taskName).fileTaskState.getTaskProgress());
        }
    }
}
