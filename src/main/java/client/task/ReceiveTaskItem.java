...
import action.RelaseAction;
import action.SendFileInfoAction;
 ...
 */
public class ReceiveTaskItem implements Runnable, InitAction, RelaseAction, ReceiveAction, SendFileInfoAction {
    ...
    public ReceiveTaskItem(String serviceFilePath, String savePeath, TaskState taskState) {
    ...
    public void init() {
        try {
            socket = new Socket(ClientConfig.SERVICE_HOST, ClientConfig.SERVICE_PORT);
    ...
    @Override
    public void receive() {
        File dirs = new File(ClientConfig.SAVE_PATH + "\\" + savePeath);
        if (!dirs.exists()) {
            dirs.mkdirs();
        }
        int fileNum = 0;
        ...
        for (int i = 0; i < fileNum; i++) {
            writeLens = 0;
            try {
                fout = new FileOutputStream(ClientConfig.SAVE_PATH + "\\" + savePeath + "\\" + fileinfos[i].getFileName());
                while (true) {
                    if (leftLen > 0) {
                        bufferedLen = leftLen;
                    } else {
